...
from .make_random_structure import *
from rlcsp.rlcsp import *
from rlcsp.state import *
import pandas
...
def mc(startup=2,sl='',output='full',A_Mods='',B_Mods='',charges='',ctype='gulp',kwds='',opts='',shel='',ratt_dist='',
       smax=250,rmax='',lout='lowest_energy_structure.cif',all_out='T',red_T=0.1,check='T',
       composition='',delay=0,min_bond=1.2,min_bond_num='',
...
,reinforce_id=0,
fixed_episodes=True,
reg_params={'e_threshold': 0, 'beta': 0, 'free_term': 0, 'h_type': 'non-linear'}
):
...
###note that the pert option needs to be entered in as a tuple of lists, for each list the first element is the pertubation type, the second is it's weighting###
    ...
    print("\n--------------------------------------------------------------")
    print("|                                                            |")
    print("|              Monte-Carlo EMMA v 4.01                       |")
    print("|                                                            |")
    print("|updated: 16th May 2021                                      |")
    print("--------------------------------------------------------------") 
...
#########################################################################################################################
    #
    # print(replace_modules(AMods, BMods, struct, layers_n=1))
    # return
...
#        for i in range(len(tmp)):
    ...
    # struct = np.array([[8,0],[7,0],[0,0],[0,0],[5,0]])
...
############# check in here to make sure that atom - atom distances are longer that min_bond (from input, with default = 1.2 A), to make sure that structures from GULP that were on the verge of collapsing are not accepted #################
    ...
    if startup != 3:
        try:    
            ...
            initial_energy = 1.0e20/len(atoms)
    if all_out=='T':
        if archive_structures != True:
    ...
    if startup ==3:
        print("\ncurrent lowest energy = %6.6e"%(initial_energy),"eV/atom, step:",idx,"\n")
    ...
    accepted_energy = initial_energy
    accepted_step = 0
    if startup != 3:
        energies={}    
    ...
    # thompson = ContextThompson(actions=["T1", "T2", "T3", "T4", "T5", "T6"],
    #                            dist_type=bandit_dist,
    #                            params_db=params_db)

    pert_list = list(pert)
    actions = [i[0] for i in pert_list]
    ...
    while s <= smax:
        ...
        # p_t = thompson.select_arm(old_energy, excluded_actions)
        # print("Thompson sampling: " + p_t)

        old_state = State(energy=accepted_energy, struct=struct)

        excluded_actions = ['T3'] if 'T3' in excluded_actions else []
        if SL is max(sl):
            excluded_actions.append('T5')
        p_t = reinforce.select_action(old_state, excluded_actions)
        print("\n\n\nReinforce: " + p_t)
        ...
        if p_t == 'T3': ### type 3: one layer in the current structure should be swapped for another, of a different type note that it should allow for the structure to maintain charge neutrality
            ...
            if types[t]=='hexagonal':
                ...
                SL = len(temp)
                swappable={'A':{},'B':{}}
                #first work out if we can swap out any modules, for A first:
                # Take into account only the modules of the structure
                for x in list(acomp.keys()):
                    if len(acomp[x]) > 1 and len(np.intersect1d(temp[:, 0], acomp[x])) > 0:
                    	  for z in acomp[x]:
                    	  	  swappable['A'][x]=acomp[x].copy()
                #then for B:
                for x in list(bcomp.keys()):
                    if len(bcomp[x]) > 1 and len(np.intersect1d(temp[:, 1], bcomp[x])) > 0:
                        for z in bcomp[x]:
                        	swappable['B'][x]=bcomp[x].copy()

                #now based on the above, choose which we're going to swap:
                swap=[0,1]
                if len(swappable['A']) == 0:
                    swap.remove(0)
                		 
                if len(swappable['B']) == 0:
                    swap.remove(1)
                    
                if len(swap) == 0: # if we can't swap anything, break the loop & move on
                	 # print("hello")
                	 p_t=choice(c_f)
                	 #print(p_t)
                	 fail = True     
                ...
                #extract the modules which we're going to swap (i.e. A or B modules)
                string=[]
                for x in range(len(temp)):
                    string.append(temp[x,to_swap])
                
                #print("temp\n",temp)
                #print("string: ",string)
                
                #now need to work out which ones from the sequence can be swapped!
                #print("swappable: ",swappable)
                if to_swap == 0:
                	 keys=list(swappable['A'].keys())
                	 
                if to_swap == 1:
                	 keys=list(swappable['B'].keys())
                	 
                #print("keys: ",keys)
                
                swaps=[] # should be the indicies of the modules which can be changed
                ...
                # print("swaps: ",swaps)
                # Now pick one of the swaps to actually swap & swap it in the string
                swap = choice(swaps)
                #print("swap: ",swap)
                ...
                while new == string[swap]:
                    if to_swap == 0:
                    	 form=AMods[string[swap]].get_chemical_formula()
                    if to_swap == 1:
                    	 form=BMods[string[swap]].get_chemical_formula()
                ...
                #now need to go back an reinsert the string into the temporary structure array:
...
### min_bond_nu,     should be a parameter to use to specifiy a minimum number of bonds for different species, expressed as a dictionary. e.g. min_bond_num = {"Al":[4,'O',3]}, note that this specifies min of 4 bonds for Al to Oxygen, with a cufoff of 3 Ang.
        ...
        if p_t in ['T7', 'T8', 'T9', 'T10', 'T11']:
            ...
            layers_n = int(SL * frac)
            print('layers_n=' + str(layers_n))

            if layers_n > 0:
                new_struct = replace_modules(AMods, BMods, struct, layers_n=layers_n, tmax=tmax)

                if type(new_struct) != numpy.ndarray:
                    continue
                else:
                    struct = new_struct

            # if layers_n=0 we do the smallest step: swap one module.
            # The code is copypasted from T3
            else:
                ...
                if types[t] == 'hexagonal':
                    ...
                    if ab == 0:  ### for swapping an A module
                        loc1a = temp[
                            swap_idx - 1, ab]  # working out what the neighbouring modules are to the module chosen for swapping
                        try:
                            loc2a = temp[swap_idx + 1, ab]
                        except(IndexError):
                            loc2a = temp[0, ab]
                        for i in range(
                                len(list(a_2_dict.keys()))):  ### this determines the translations of the two neighbours
                            if loc1a in a_2_dict[i]:
                                loc1b = i
                            if loc2a in a_2_dict[i]:
                                loc2b = i
                        if loc1b != loc2b:  ## if neighbouring layers are different translations, then the new module must be of the same translation
                            ...
                            trial = 0
                            while temp[swap_idx, ab] == swap_mod and trial <= 500:
                                swap_mod = choice(a_2_dict[
                                                      loc3])  ### swaps the A module to a A module from the input file, inserting a new module of the same translation, since the translation is fixed by the neighbouring A-site layers
                                trial = trial + 1
                            #    if trial == 500:
                            #        continue
                    ...
                    if ab == 1:
                        for i in range(len(
                                list(b_2_dict.keys()))):  ### determines the translation of the interstitial site layer
                            if temp[swap_idx, ab] in b_2_dict[i]:
                                loc1 = i
                        swap_mod = choice(b_2_dict[loc1])
                        trial = 0
                        while temp[swap_idx, ab] == swap_mod and trial <= 500:
                            swap_mod = choice(b_2_dict[
                                                  loc1])  ### swaps the interstitial module to a different interstitial from the input file, inserting a new module of the same translation, since the translation is fixed by the corrisponding A-site layers
                            trial = trial + 1
                            # if trial == 500:
                            #    continue
                        temp[swap_idx, ab] = swap_mod
                    ...
                    swappable = {'A': {}, 'B': {}}
                    # first work out if we can swap out any modules, for A first:
                    # Elena: take into account only the modules of the structure
                    for x in list(acomp.keys()):
                        if len(acomp[x]) > 1 and len(np.intersect1d(temp[:, 0], acomp[x])) > 0:
                            for z in acomp[x]:
                                swappable['A'][x] = acomp[x].copy()
...
#####################################################################################################
        ...
        diff = float(new_energy - accepted_energy)
        # if s >> 1:
        #     l_conf_old=l_conf
        ...
        if new_energy == min(energies, key=energies.get):
        ...
        accepted_old = accepted_step
        # if l_conf == (s+p_s):
        if diff < 0:
            accepted_structure = new_atoms.copy()
            accepted_energy = new_energy
            accepted_step = s + p_s
            write(lout, accepted_structure)
            if output == 'full':
                #write("accepted/"+str(s+p_s)+".cif",l_conf_atoms)
                if archive_structures != True:
                    write("accepted/"+str('{0:0=5n}'.format(s+p_s)+".cif"), accepted_structure)
            accepted.append(struct)
            outcome = 'A'
            if outcome == 'A':
            ...
            if diff == 0.:
                ...
                if chance >= p:
                    ...
                    if output == 'full':
                        #write("accepted/"+str(s+p_s)+".cif",l_conf_atoms)
                        if archive_structures != True:
                            write("accepted/"+str('{0:0=5n}'.format(s+p_s)+"MCA.cif"),accepted_structure)
                        if new_energy < 1.0e+5:
        ...
        # if we accept the structure then we don't need to exclude T3
        if accepted_step == s + p_s:
            excluded_actions = []

        new_state = State(new_energy, struct)
        print(old_state)
        print(new_state)
        # end the episode if the structure rejected and fixed_episodes is False
        if outcome not in ['A', 'mcA'] and not fixed_episodes:
            reinforce.end_episode()
            reinforce.update(p_t, old_state, new_state, end_episode=True)
        else:
            reinforce.update(p_t, old_state, new_state)
        ...
        print(str(str(s+p_s).ljust(5)+str('{0:4s}'.format(str("NE = ")))+str('{0: 6.4e}'.format(new_energy).ljust(11)) \
                  + str(" eV/atom dE = {0: 6.4e}".format(diff).ljust(6)) \
                  + str(" vs.")+str('{0:4n}'.format(accepted_old).ljust(4)) \
                  + str(" pert:")+str(p_t)+str(" Out: {0:3s}".format(outcome).ljust(3)) \
                  + str(" CR: {0:3n}".format(r).ljust(5))), end=' ')

        if dump =='T':
            o.write(str(str(s+p_s).ljust(5)+str('{0:4s}'.format(str("NE = "))) \
                        + str('{0: 6.4e}'.format(new_energy).ljust(11)) \
                        + str(" eV/atom dE = {0: 6.4e}".format(diff).ljust(6)) \
                        + str(" vs.")+str('{0:4n}'.format(accepted_old).ljust(4)) \
                        + str(" pert:")+str(p_t)+str(" Out: {0:3s}".format(outcome).ljust(3)) \
                        + str(" CR: {0:3n}".format(r).ljust(5))))
...
#        print "CPU: %3.1f"%(t_s)
        ...
        tmpaccepted = numpy.array(accepted,dtype='object')
...
#        numpy.savez("permitted.npz",permitted=permitted)
    ...
    perms = numpy.array(perms,dtype='object')
